...
function requireTsLoader () {
	...
	const { createRequire } = require$$0__default;
...
async function loadProgramProcessor(type = "ts") {
  if (type === "vue") {
    try {
      const mod = await import('../chunks/vue.cjs');
  ...
  try {
    const mod = await import('../chunks/ts.cjs');
    return mod.default ?? mod;
  } catch {
    const mod = await import('../chunks/ts.cjs');
    return mod.default ?? mod;
...
function tryGetPackageInfo(name) {
  if (process.versions.pnp) {
    const targetRequire = require$$0.createRequire((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('shared/unplugin-dts.CpHGZJg0.cjs', document.baseURI).href)));
    try {
...
function transformAlias(importer, dir, aliases, aliasesExclude) {
  if (aliases?.length && !aliasesExclude.some((e) => isRegExp(e) ? e.test(importer) : String(e) === importer)) {
    for (const alias of aliases) {
      ...
      const replacement = node_path.isAbsolute(alias.replacement) ? normalizePath(node_path.relative(dir, alias.replacement)) : normalizePath(alias.replacement);
      const endsWithSlash = typeof alias.find === "string" ? alias.find.endsWith("/") : importer.match(alias.find)[0].endsWith("/");
...
class Runtime {
  ...
  constructor(options, { createParsedCommandLine, createProgram }) {
    ...
    const resolvedBaseUrl = baseUrl ?? (paths && configPath ? node_path.dirname(configPath) : root);
    ...
    const computeGlobs = (rootGlobs, tsGlobs, defaultGlob) => {
      ...
      const relativeRoot = configPath ? normalizePath(node_path.relative(root, node_path.dirname(configPath))) : ".";
  ...
  async emitOutput(options = {}) {
    ...
    for (const sourceFile of sourceFiles) {
      ...
      if ((copyDtsFiles || bundleTypes) && dtsRE$1.test(sourceFile.fileName)) {
    ...
    await runParallel(
      ...
      async ([filePath, content]) => {
        let relativePath = node_path.relative(
          entryRoot,
          cleanVueFileName ? cleanVueDtsFileName(filePath) : filePath
        );
        if (relativePath.startsWith("..")) {
          relativePath = node_path.relative(root, cleanVueFileName ? cleanVueDtsFileName(filePath) : filePath);
        }
    ...
    if (insertTypesEntry || bundleTypes) {
      ...
      const transformed = new Set(
        Array.from(transformedFiles).map((file) => {
          file = node_path.relative(entryRoot, file);
      ...
      for (const name of entryNames) {
        ...
        const sourceEntry = normalizePath(
          cleanPath(
            resolve(
              outDir,
              node_path.relative(entryRoot, tsToDtsWithExtension(entries[name], primaryDtsExtension))
            ),
            emittedFiles
          )
        );
        if (entryDtsPath === sourceEntry || transformed.has(node_path.relative(outDir, entryDtsPath)) && node_fs.existsSync(entryDtsPath))
          continue;
        let fromPath = normalizePath(node_path.relative(node_path.dirname(entryDtsPath), sourceEntry));
        fromPath = fromPath.replace(dtsRE$1, (_, prefix) => `.${prefix || ""}js`);
...
const pluginFactory = (options = {}, meta) => {
  ...
  function hasVueFilesInDir(dir) {
    try {
      const entries2 = node_fs.readdirSync(dir, { withFileTypes: true });
      for (const entry of entries2) {
        if (entry.isFile() && entry.name.endsWith(".vue")) {
          return true;
        }
        if (entry.isDirectory() && !entry.name.startsWith(".") && entry.name !== "node_modules") {
          const subEntries = node_fs.readdirSync(`${dir}/${entry.name}`, { withFileTypes: true });
          ...
          entries = entryPoints.reduce(
            (prev, current) => {
              if (typeof current === "string") {
                ...
                prev[node_path.basename(current.in)] = current.out;
              }
              return prev;
            },
            {}

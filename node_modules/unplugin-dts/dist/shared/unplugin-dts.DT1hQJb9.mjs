...
async function loadProgramProcessor(type = "ts") {
  if (type === "vue") {
    try {
      const mod = await import('../chunks/vue.mjs');
  ...
  try {
    const mod = await import('../chunks/ts.mjs');
    return mod.default ?? mod;
  } catch {
    const mod = await import('../chunks/ts.mjs');
    return mod.default ?? mod;
...
`
      ...
      for (const name of entryNames) {
        ...
        if (entryDtsPath === sourceEntry || transformed.has(relative(outDir, entryDtsPath)) && existsSync(entryDtsPath))
          continue;
